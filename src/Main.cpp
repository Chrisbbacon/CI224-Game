...
vector<shared_ptr<GameAsset> > spikes ;

bool horrible_global_go = false;
int spawnSpikes = rand() % 49 + 1;
...
int main(int argc, char ** argv) {
	...
	//Player
   	shared_ptr<CubeAsset> p = shared_ptr<CubeAsset> (shared_ptr<CubeAsset> (new CubeAsset(0, -2, 0)));
	assets.push_back(p);

	//Spikes
	for(int i = 0; i < 10; i++)
	{	
	  assets.push_back(shared_ptr<TriangularPyramidAsset> (new TriangularPyramidAsset(-10 + i, 10 + i, 60 + i)));
	}
	
	//assets.push_back(shared_ptr<CubeAsset> (new CubeAsset(5, 5, 5)));


	// Set the camera
	//Camera::getInstance().lookAt(Point3(0.0, 0.0, -10.0), Point3(0.0, 0.0, -1.0), Vector3(0.0, 1.0, 0.0));
	  Camera::getInstance().setCamera(Camera::getInstance().getCameraM() * Matrix4::translation(Vector3(0.0, 0.0, 10.0)));
		//display();
	...
	while (SDL_WaitEvent(&event)) {
			...
			case SDL_KEYDOWN:
			  ...
			  case SDLK_LEFT:
			    Camera::getInstance().setCamera((camera * Matrix4::rotationZ(15.0/180.0)));
			    break;
			  case SDLK_RIGHT:
			    Camera::getInstance().setCamera(camera * Matrix4::rotationZ(-15.0/180.0) );
			    break;
			  case SDLK_UP:
			    Camera::getInstance().setCamera((camera * Matrix4::rotationX(15.0/180.0)));
			    break;
			  case SDLK_DOWN:
			    Camera::getInstance().setCamera((camera * Matrix4::rotationX(-15.0/180.0)));
			    break;
